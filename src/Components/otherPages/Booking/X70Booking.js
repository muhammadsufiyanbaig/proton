...
import X70BookingBanner from '../../../assets/images/Booking/x70Booking.png';
import './booking.css';
...
class Booking extends Component {
  ...
  _handleSubmit() {
    ...
    if (data.firstName === "") {
      this.setState({ error: "FirstName is Required" })
    }
      ...
      axios.post(`https://proton-server.herokuapp.com/data/X70formdata`, data)
      //  axios.post(`http://localhost:8080/data/X70formdata`, data)
        .then(res => {
  ...
  render() {
    return (
      <div className="bookingMain">
        <div>
          <img src={X70BookingBanner} alt="X70BookingBanner" height="90%" width="100%" />
        </div>


        <div style={{ width: "80%", marginLeft: "10%", paddingBottom: "100px" }}>

          <div className="heading fontstyle" style={{ fontSize: "3vw", letterSpacing: "-2px" }}>
            Book your <span className="fontstyle2">X70</span>
          </div>
                ...
                <Grid item xs={12} sm={5}>
                  <div class="form-group">
                    ...
                    <select onChange={(event) => this.setState({ varient: event.target.value })} class="form-control selector fontstyle">
                      ...
                      <option value="Premium 2WD">Premium 2WD</option>
                    </select>
                  </div>
                </Grid>
                <Grid item xs={0} sm={1} />
